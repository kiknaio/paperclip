/**
 * esbuild configuration for building the yawnlessai CLI for npm.
 *
 * Bundles all workspace packages (@yawnlessai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @yawnlessai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@yawnlessai/")) {
      externals.add(name);
    }
  }
