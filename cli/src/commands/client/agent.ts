...
function buildAgentEnvExports(input: {
  ...
  return [
    `export YAWNLESS_API_URL='${escaped(input.apiBase)}'`,
    `export YAWNLESS_COMPANY_ID='${escaped(input.companyId)}'`,
    `export YAWNLESS_AGENT_ID='${escaped(input.agentId)}'`,
    `export YAWNLESS_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Yawnless.ai skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Yawnless.ai skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            ...
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Yawnless.ai skills directory. Expected ./skills in the repo checkout.",
              );
            }
