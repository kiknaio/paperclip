...
const ONBOARD_ENV_KEYS = [
  "YAWNLESS_PUBLIC_URL",
  "DATABASE_URL",
  "YAWNLESS_DB_BACKUP_ENABLED",
  "YAWNLESS_DB_BACKUP_INTERVAL_MINUTES",
  "YAWNLESS_DB_BACKUP_RETENTION_DAYS",
  "YAWNLESS_DB_BACKUP_DIR",
  "YAWNLESS_DEPLOYMENT_MODE",
  "YAWNLESS_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "YAWNLESS_ALLOWED_HOSTNAMES",
  "YAWNLESS_AUTH_BASE_URL_MODE",
  "YAWNLESS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "YAWNLESS_STORAGE_PROVIDER",
  "YAWNLESS_STORAGE_LOCAL_DIR",
  "YAWNLESS_STORAGE_S3_BUCKET",
  "YAWNLESS_STORAGE_S3_REGION",
  "YAWNLESS_STORAGE_S3_ENDPOINT",
  "YAWNLESS_STORAGE_S3_PREFIX",
  "YAWNLESS_STORAGE_S3_FORCE_PATH_STYLE",
  "YAWNLESS_SECRETS_PROVIDER",
  "YAWNLESS_SECRETS_STRICT_MODE",
  "YAWNLESS_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const publicUrl =
    process.env.YAWNLESS_PUBLIC_URL?.trim() ||
    process.env.YAWNLESS_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.YAWNLESS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.YAWNLESS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.YAWNLESS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.YAWNLESS_ALLOWED_HOSTNAMES
    ? process.env.YAWNLESS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.YAWNLESS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.YAWNLESS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.YAWNLESS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.YAWNLESS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.YAWNLESS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.YAWNLESS_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.YAWNLESS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.YAWNLESS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.YAWNLESS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.YAWNLESS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.YAWNLESS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.YAWNLESS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.YAWNLESS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.YAWNLESS_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.YAWNLESS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "YAWNLESS_DEPLOYMENT_EXPOSURE",
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" yawnlessai onboard ")));
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@yawnlessai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `yawnlessai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("YAWNLESS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.YAWNLESS_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("YAWNLESS_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("YAWNLESS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }
  ...
  p.note(
    [
      ...
      "Agent auth: YAWNLESS_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("yawnlessai run")}`,
      `Reconfigure later: ${pc.cyan("yawnlessai configure")}`,
      `Diagnose setup: ${pc.cyan("yawnlessai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.YAWNLESS_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("yawnlessai run")}`,
        `Then: ${pc.cyan("yawnlessai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
