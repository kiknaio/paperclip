...
} from "@yawnlessai/adapter-utils";
import { asString, parseObject } from "@yawnlessai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Yawnless.ai server host.",
        });
      }
    } catch (err) {
      checks.push({
