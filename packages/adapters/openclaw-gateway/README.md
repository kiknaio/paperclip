...
This document describes how `@yawnlessai/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Yawnless.ai logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Yawnless.ai `runId`)
