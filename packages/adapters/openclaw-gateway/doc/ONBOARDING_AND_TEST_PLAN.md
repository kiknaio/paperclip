...
## Scope
This plan is now **gateway-only**. Yawnless.ai supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Yawnless.ai skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Yawnless.ai task.
...
```bash
pnpm --filter @yawnlessai/server test:run -- openclaw-gateway
pnpm --filter @yawnlessai/server typecheck
pnpm --filter @yawnlessai/ui typecheck
pnpm --filter yawnlessai typecheck
```
