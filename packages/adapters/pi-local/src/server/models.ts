...
import type { AdapterModel } from "@yawnlessai/adapter-utils";
import { asString, runChildProcess } from "@yawnlessai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.YAWNLESS_PI_COMMAND === "string" &&
    process.env.YAWNLESS_PI_COMMAND.trim().length > 0
      ? process.env.YAWNLESS_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["YAWNLESS_", "npm_", "NPM_"] as const;
