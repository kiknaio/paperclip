...
Use when:
- You want Yawnless.ai to run Cursor Agent CLI locally as the agent runtime
...
- Yawnless.ai auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$yawnless" and related skills on local runs.
- Yawnless.ai auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
