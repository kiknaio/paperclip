...
type PaperclipSprite = (typeof YAWNLESS_SPRITES)[number];
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = YAWNLESS_SPRITES[(baseCol + baseRow) % YAWNLESS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = YAWNLESS_SPRITES[Math.floor(Math.random() * YAWNLESS_SPRITES.length)]!;
