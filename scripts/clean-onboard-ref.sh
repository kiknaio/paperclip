...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Yawnless.ai data dir to use
  YAWNLESS_HOST=127.0.0.1    Host passed to the onboarded server
  YAWNLESS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/yawnless-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/yawnless-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export YAWNLESS_HOST="${YAWNLESS_HOST:-127.0.0.1}"
export YAWNLESS_PORT="${YAWNLESS_PORT:-3100}"
export YAWNLESS_OPEN_ON_LISTEN="${YAWNLESS_OPEN_ON_LISTEN:-false}"
...
echo "YAWNLESS_HOST: $YAWNLESS_HOST"
echo "YAWNLESS_PORT: $YAWNLESS_PORT"
...
env \
  ...
  HOST="$YAWNLESS_HOST" \
  PORT="$YAWNLESS_PORT" \
  YAWNLESS_OPEN_ON_LISTEN="$YAWNLESS_OPEN_ON_LISTEN" \
  pnpm yawnlessai onboard --yes --data-dir "$PC_DATA"
