...
YAWNLESS_API_URL="${YAWNLESS_API_URL:-http://127.0.0.1:3100}"
API_BASE="${YAWNLESS_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-yawnless-smoke}"
...
YAWNLESS_API_URL_FOR_OPENCLAW="${YAWNLESS_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${YAWNLESS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${YAWNLESS_AUTH_HEADER}" )
fi
if [[ -n "${YAWNLESS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${YAWNLESS_COOKIE}" )
  YAWNLESS_BROWSER_ORIGIN="${YAWNLESS_BROWSER_ORIGIN:-${YAWNLESS_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${YAWNLESS_BROWSER_ORIGIN}" -H "Referer: ${YAWNLESS_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set YAWNLESS_COOKIE or YAWNLESS_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg paperclipApiUrl "$YAWNLESS_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/yawnless"
  local claimed_file="${workspace_dir}/yawnless-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/yawnless-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/yawnless-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/yawnless-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/yawnless"
    assert_status "200"
    {
      echo "YAWNLESS_API_URL=${YAWNLESS_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set YAWNLESS_API_KEY=${AGENT_API_KEY} in your run context before running Yawnless.ai heartbeat steps."
...
main() {
  ...
  wait_http_ready "${YAWNLESS_API_URL%/}/api/health" 15 || fail "Yawnless.ai API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "yawnless health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
