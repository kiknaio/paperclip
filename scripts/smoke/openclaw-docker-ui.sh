...
YAWNLESS_HOST_PORT="${YAWNLESS_HOST_PORT:-3100}"
YAWNLESS_HOST_FROM_CONTAINER="${YAWNLESS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.yawnless-openclaw.override.yml"
...
detect_paperclip_base_url() {
  ...
  for candidate in "$YAWNLESS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${YAWNLESS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${YAWNLESS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
Yawnless.ai URL for OpenClaw container:
EOF
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${YAWNLESS_HOST_PORT}
  (Do not use http://127.0.0.1:${YAWNLESS_HOST_PORT} inside the container.)
  If Yawnless.ai rejects the host, run on host machine:
    pnpm yawnlessai allowed-hostname host.docker.internal
  Then restart Yawnless.ai and re-run this script.
EOF
  fi
  cat <<EOF
