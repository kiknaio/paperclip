...
COMPANY_ID="${COMPANY_ID:-${YAWNLESS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-yawnless-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-yawnless-openclaw-smoke}"
...
Provide one of:
  YAWNLESS_AUTH_HEADER=\"Bearer <board-token>\"
  YAWNLESS_COOKIE=\"<board-session-cookie>\"
...
log "checking Yawnless.ai health"
...
assert_status "200"
if ! grep -q "Yawnless.ai OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.yawnless.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
