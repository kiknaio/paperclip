...
Audience: Operators setting up and running agents in Yawnless.ai

## 1. What this system does

Agents in Yawnless.ai do not run continuously.  
...
For `claude_local` and `codex_local`, Yawnless.ai assumes the CLI is already installed and authenticated on the host machine.
...
Yawnless.ai stores session IDs for resumable adapters.
...
Yawnless.ai pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Yawnless.ai surfaces this as a warning in environment tests, not a hard error.
