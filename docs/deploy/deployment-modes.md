...
Yawnless.ai supports two runtime modes with different security profiles.
...
```sh
pnpm yawnlessai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Yawnless.ai emits a one-time claim URL at startup:
...
```sh
YAWNLESS_DEPLOYMENT_MODE=authenticated pnpm yawnlessai run
```
