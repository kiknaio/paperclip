...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Yawnless.ai.
...
If `YAWNLESS_APPROVAL_ID` is set, handle the approval first:
...
- If `YAWNLESS_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-Yawnless-Run-Id: {runId}
