...
How to get OpenClaw running in a Docker container for local development and testing the Yawnless.ai OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Yawnless.ai includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-yawnless-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Yawnless.ai host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-yawnless-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `YAWNLESS_HOST_PORT` (default `3100`)
- `YAWNLESS_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Yawnless.ai deployment is `authenticated`, provide auth context:

```bash
YAWNLESS_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
YAWNLESS_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Yawnless.ai server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Yawnless.ai URL (typically `http://host.docker.internal:3100`).
- If Yawnless.ai rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm yawnlessai allowed-hostname host.docker.internal
```

Then restart Yawnless.ai and rerun the smoke script.
