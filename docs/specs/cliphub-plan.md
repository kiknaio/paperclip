# ClipHub: Marketplace for Yawnless.ai Team Configurations

> The "app store" for whole-company AI teams — pre-built Yawnless.ai configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Yawnless.ai-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Yawnless.ai company (agents, projects, governance) |
...
A complete Yawnless.ai company configuration:
...
Single-agent configurations designed to plug into a Yawnless.ai org:
...
Portable skill files that any Yawnless.ai agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Yawnless.ai's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Yawnless.ai version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Yawnless.ai company
...
| `POST` | `/api/listings/:id/install` | Install to Yawnless.ai company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Yawnless.ai company (or create new)
  → ClipHub API calls Yawnless.ai API to:
  ...
  → Redirect to Yawnless.ai dashboard with new team running
```
...
### 5.3 Creator: Export from Yawnless.ai → Publish

```
Running Yawnless.ai company → "Export as Blueprint" (CLI or UI)
  → Yawnless.ai exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Yawnless.ai brand blue/purple)
...
| Install flow | One-click deploy to Yawnless.ai company |
...
{
  "targetCompanyId": "uuid",         // Existing Yawnless.ai company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `yawnless-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Yawnless.ai
- **Backend**: Node.js API (or extend Yawnless.ai server)
- **Database**: Postgres (can share Yawnless.ai's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Yawnless.ai auth (or OAuth2)

### 9.2 Integration with Yawnless.ai

ClipHub can be:
- **Option A**: A separate app that calls Yawnless.ai's API to install blueprints
- **Option B**: A built-in section of the Yawnless.ai UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Yawnless.ai UI and API.
...
- [ ] Install flow: blueprint → Yawnless.ai company
...
- [ ] "Export from Yawnless.ai" CLI command
