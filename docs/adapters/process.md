...
- Running a Python script that calls the Yawnless.ai API
...
1. Yawnless.ai spawns the configured command as a child process
2. Standard Yawnless.ai environment variables are injected (`YAWNLESS_AGENT_ID`, `YAWNLESS_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Yawnless.ai API and perform work.
