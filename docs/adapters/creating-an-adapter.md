...
Build a custom adapter to connect Yawnless.ai to any agent runtime.
...
`format-event.ts` — pretty-prints stdout for `yawnlessai run --watch` using `picocolors`.
...
Make Yawnless.ai skills discoverable to your agent runtime without writing to the agent's working directory:
