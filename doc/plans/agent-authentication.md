...
Agents need API keys to authenticate with Yawnless.ai. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Yawnless.ai
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Yawnless.ai generates a token and passes it directly to the agent
...
`YAWNLESS_API_URL`, `YAWNLESS_AGENT_ID`, `YAWNLESS_COMPANY_ID`. We need to
add a `YAWNLESS_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Yawnless.ai CLI
...
```
yawnless auth login
# Opens browser -> user confirms -> token stored at ~/.yawnless/credentials
```
...
managed by the Yawnless.ai server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Yawnless.ai.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Yawnless.ai SKILL.md (or a link to it)
   - What information Yawnless.ai needs from the agent (e.g. webhook URL, adapter
...
5. Yawnless.ai stores the pending registration.
...
7. On approval, Yawnless.ai provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Yawnless.ai issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.yawnless.ai/skills/yawnless/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Yawnless.ai should send you heartbeats",
...
1. Generate a Yawnless.ai invite link for the company.
...
4. A Yawnless.ai company member approves the new agent.
5. Yawnless.ai begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `YAWNLESS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `yawnlessai auth login` for developer-managed remote agents.                                      |
