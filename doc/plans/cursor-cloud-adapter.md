...
This document defines the V1 design for a Yawnless.ai adapter that integrates with
...
4. synthesized stdout events for Yawnless.ai UI/CLI
...
3. **Agent callback auth to Yawnless.ai** uses a bootstrap exchange flow (no long-lived Yawnless.ai key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Yawnless.ai endpoints, not full SKILL.md prompt injection.
...
- `paperclipPublicUrl` (optional override; else `YAWNLESS_PUBLIC_URL` env)
...
## Yawnless.ai Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `YAWNLESS_API_KEY`.
...
2. `process.env.YAWNLESS_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Yawnless.ai credentials in prompt text.

Flow:

1. Before launch/follow-up, Yawnless.ai mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Yawnless.ai validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Yawnless.ai API calls.
...
1. Prompt includes a compact instruction to fetch skills from Yawnless.ai.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/yawnless`
   - `GET /api/skills/yawnless-create-agent` when needed
...
4. Route by Cursor `agentId` to active Yawnless.ai run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from Yawnless.ai skill endpoints |
| Yawnless.ai auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Yawnless.ai context.
3. Add richer PR metadata surfacing in Yawnless.ai UI.
