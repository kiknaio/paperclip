...
Add a single storage subsystem for Yawnless.ai that supports:
...
- API serves attachment bytes through authenticated Yawnless.ai endpoints.
...
- Default local path is under instance root: `~/.yawnless/instances/<instanceId>/data/storage`.
...
- `yawnlessai onboard` writes a valid `storage` config block by default.
- `yawnlessai configure --section storage` can switch between local and s3 modes.
...
- `yawnlessai doctor` reports actionable storage status.
