...
- New config/env settings:
  - `YAWNLESS_AGENT_JWT_SECRET`
  - `YAWNLESS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `YAWNLESS_AGENT_JWT_ISSUER` (default: `yawnless`)
  - `YAWNLESS_AGENT_JWT_AUDIENCE` (default: `yawnless-api`)
...
1. In:
   ...
   inject `YAWNLESS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `YAWNLESS_API_KEY`, do not overwrite it.
- Continue injecting:
  - `YAWNLESS_AGENT_ID`
  - `YAWNLESS_COMPANY_ID`
  - `YAWNLESS_API_URL`
