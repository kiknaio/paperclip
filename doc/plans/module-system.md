# Yawnless.ai Module System

## Overview

Yawnless.ai's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Yawnless.ai's API, UI, and data model | Yes |
...
modules/
  observability/
    yawnless.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@yawnlessai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@yawnlessai/core";
...
```
1. Scan modules/ directory for yawnless.module.json manifests
...
```jsonc
// yawnless.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@yawnlessai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@yawnlessai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:yawnless/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.yawnless.ai/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm yawnlessai store list                    # browse available modules and templates
pnpm yawnlessai store install <module-id>     # install a module
pnpm yawnlessai store import <template-id>    # import a company template
pnpm yawnlessai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Yawnless.ai instance | `server:started` |
...
4. **Module config** — `yawnless.config.json` with per-module config, env var interpolation
...
10. **`@yawnlessai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
