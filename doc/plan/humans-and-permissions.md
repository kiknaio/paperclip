...
- local mode that still feels instant (`npx yawnlessai run` and go)
...
2. CLI command `pnpm yawnlessai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm yawnlessai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm yawnlessai onboard`).
...
- expose key instance settings in API + CLI (`yawnlessai configure` / `yawnlessai onboard`)
...
7. `pnpm yawnlessai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
