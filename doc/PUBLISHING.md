...
Low-level reference for how Yawnless.ai packages are built for npm.
...
The CLI package, `yawnlessai`, imports code from workspace packages such as:
...
- `npx yawnlessai@canary onboard` can install them explicitly
- `npx yawnlessai onboard` continues to resolve `latest`
