...
Yawnless.ai V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Yawnless.ai-shaped design.

Assumption for this document: Yawnless.ai is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Yawnless.ai objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Yawnless.ai.

The main conclusion is:

- Yawnless.ai should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Yawnless.ai should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Yawnless.ai should use multiple extension classes instead of one generic plugin bag:
...
If Yawnless.ai does this well, the examples you listed become straightforward:
...
Relevant Yawnless.ai files reviewed for current extension seams:
...
It is too dangerous for Yawnless.ai core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Yawnless.ai — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Yawnless.ai should absolutely do this.
...
Yawnless.ai should copy this discipline.
...
Yawnless.ai should adopt this style for plugin actions, automations, and UI schemas.
...
Yawnless.ai can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Yawnless.ai too.

## What Yawnless.ai Should Not Copy Directly
...
Yawnless.ai is a control plane for an operator-managed instance with company objects.
...
Yawnless.ai is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Yawnless.ai should not auto-load arbitrary code from a workspace repo like `.yawnless/plugins` or project directories.
...
For Yawnless.ai, equivalent hooks into:
...
Yawnless.ai should not allow plugins to silently replace:
...
For Yawnless.ai it would be risky because it combines:
...
Yawnless.ai should require an explicit operator install step.

## Why Yawnless.ai Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Yawnless.ai |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Yawnless.ai instance |
...
That means Yawnless.ai should borrow the good ideas from `opencode` but use a stricter architecture.

## Yawnless.ai Already Has Useful Pre-Plugin Seams

Yawnless.ai has several extension-like seams already:
...
This is good news.
Yawnless.ai does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Yawnless.ai Plugin Model
...
Yawnless.ai already has this pattern implicitly:
...
- `plugin` means integration code that talks to Yawnless.ai through a typed plugin protocol and capability model
...
- invoke explicit Yawnless.ai actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@yawnlessai/plugin-sdk/ui`) with design tokens and common components.
...
Yawnless.ai should treat plugin installation as a global instance-level action.

Examples:

- install `@yawnless/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Yawnless.ai objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Yawnless.ai already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Yawnless.ai too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@yawnless/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@yawnlessai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Yawnless.ai instance.

A `create-yawnless-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Yawnless.ai achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Yawnless.ai server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Yawnless.ai's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@yawnlessai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@yawnlessai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Yawnless.ai

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@yawnlessai/plugin-sdk";

export default definePlugin({
  id: "@yawnless/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Yawnless.ai entities
    });
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@yawnlessai/plugin-sdk/ui";
...
Any Yawnless.ai plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Yawnless.ai objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@yawnless/plugin-linear`)
...
- reuse core tables when the data is clearly part of Yawnless.ai itself
...
If a concept is becoming part of Yawnless.ai's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Yawnless.ai itself
...
### 3. Scope plugin data to Yawnless.ai objects before adding custom schemas

A lot of plugin data naturally hangs off existing Yawnless.ai objects:
...
If Yawnless.ai eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@yawnless/plugin-workspace-files`
...
Package idea: `@yawnless/plugin-terminal`
...
Package idea: `@yawnless/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Yawnless.ai.
- Board reviews the diff after a run without leaving Yawnless.ai.
...
- optional `events.emit` (e.g. `plugin.@yawnless/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@yawnless/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@yawnless/plugin-linear`

This plugin syncs Yawnless.ai work with Linear. It is useful for:

- importing backlog from Linear
- linking Yawnless.ai issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Yawnless.ai and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Yawnless.ai.
- Yawnless.ai issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@yawnless/plugin-github-issues`

This plugin syncs Yawnless.ai issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Yawnless.ai issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Yawnless.ai task governance
...
- Plugin settings:
  ...
  - whether new Yawnless.ai issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Yawnless.ai issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Yawnless.ai.
- GitHub webhooks update status/comment state in Yawnless.ai.
- A PR is linked back to the Yawnless.ai issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@yawnless/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@yawnless/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Yawnless.ai. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Yawnless.ai issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Yawnless.ai plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Yawnless.ai dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Yawnless.ai issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Yawnless.ai core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@yawnless/plugin-runtime-processes`
...
Package idea: `@yawnless/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Yawnless.ai. It is useful for:
...
- Failed payment spikes or churn events can generate Yawnless.ai issues for follow-up.
...
- Stripe data should stay additive to Yawnless.ai core
...
- plugin UI bundle loading, host bridge, `@yawnlessai/plugin-sdk/ui`
...
- `@yawnlessai/plugin-test-harness` and `create-yawnless-plugin` starter template
...
Yawnless.ai should not implement "an OpenCode-style generic in-process hook system."
Yawnless.ai should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Yawnless.ai
...
8. Ship `@yawnlessai/plugin-test-harness` and `create-yawnless-plugin` starter template.
