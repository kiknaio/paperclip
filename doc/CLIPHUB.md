...
ClipHub is the public registry where people share, discover, and download Yawnless.ai company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Yawnless.ai what a package registry is to a programming language. Yawnless.ai already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Yawnless.ai — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Yawnless.ai instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Yawnless.ai spec. It contains:
...
**Install (fresh start):**
```
yawnless install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Yawnless.ai instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Yawnless.ai, export your company as a template, then publish:

```
yawnless export --template my-company
yawnless publish cliphub my-company
```
...
ClipHub is a **separate service** from Yawnless.ai itself. Yawnless.ai is self-hosted; ClipHub is a hosted registry that Yawnless.ai instances talk to.
...
| **Yawnless.ai CLI** | `yawnlessai install`, `yawnlessai publish`, `yawnlessai cliphub sync` — built into Yawnless.ai |
| **Yawnless.ai UI** | "Browse ClipHub" panel in the Yawnless.ai web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Yawnless.ai) |
| Backend | TypeScript + Hono (consistent with Yawnless.ai) |
| Database | PostgreSQL |
...
4. Run `yawnlessai install cliphub:acme/lean-saas-shop`
5. Yawnless.ai creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Yawnless.ai until it works well
2. Export: `yawnlessai export --template my-agency`
3. Publish: `yawnlessai publish cliphub my-agency`
...
3. Install just that agent: `yawnlessai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Yawnless.ai

ClipHub is **not required** to use Yawnless.ai. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Yawnless.ai what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `yawnlessai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Yawnless.ai web UI
- [ ] `yawnlessai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Yawnless.ai's own philosophy)
