...
This spec defines how Yawnless.ai actually runs agents while staying runtime-agnostic.
...
1. Yawnless.ai is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Yawnless.ai manages lifecycle, and on exit Yawnless.ai parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `yawnless.skill` (shared Yawnless.ai skill text block)
...
2. Preferred transport is env vars (`YAWNLESS_*`) injected at runtime.
