# Releasing Yawnless.ai

Maintainer runbook for shipping a full Yawnless.ai release across npm, GitHub, and the website-facing changelog surface.
...
2. **npm** — `yawnlessai` and public workspace packages are published
...
```bash
VERSION=X.Y.Z
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Yawnless.ai. Read doc/RELEASING.md and skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
Yawnless.ai uses one release branch per target stable version:
...
./scripts/release-start.sh minor --worktree-dir ../yawnless-release-0.4.0
...
YAWNLESS_PORT=3234 ./scripts/clean-onboard-ref.sh
./scripts/clean-onboard-ref.sh HEAD
```

Minimum checks:

- `npx yawnlessai@canary onboard` installs
...
- [ ] `npm view yawnlessai@latest version` matches the new stable version
