...
import { agents as agentsTable, companies, heartbeatRuns } from "@yawnlessai/db";
...
import { runClaudeLogin } from "@yawnlessai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@yawnlessai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@yawnlessai/adapter-cursor-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@yawnlessai/adapter-opencode-local/server";
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.YAWNLESS_SECRETS_STRICT_MODE === "true";
