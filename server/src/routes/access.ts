...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (normalized !== "yawnless" && normalized !== "yawnless-create-agent")
    return null;
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Yawnless.ai host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Yawnless.ai is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm yawnlessai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/yawnless";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/yawnless-claimed-api-key.json and load YAWNLESS_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Yawnless.ai hostname is reachable and allowed via `pnpm yawnlessai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Yawnless.ai API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/yawnless/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Yawnless.ai OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Yawnless.ai generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "paperclipApiUrl": "https://yawnless-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Yawnless.ai before key claim is allowed.
    ...
    - ~/.openclaw/workspace/yawnless-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/yawnless-claimed-api-key.json

    And set the YAWNLESS_API_KEY and YAWNLESS_API_URL in your environment variables as specified here:
    ...
    Then set YAWNLESS_API_KEY and YAWNLESS_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Yawnless.ai skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your YAWNLESS_API_URL to the top of your skill and note the path to your YAWNLESS_API_URL
    ...
    ${
      ...
      "Ensure Yawnless.ai is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Yawnless.ai base URLs to try");
    ...
    appendBlock(`
      ...
      - pnpm yawnlessai allowed-hostname <host>
      - then restart Yawnless.ai and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@yawnless.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "yawnless", path: "/api/skills/yawnless" },
        {
          name: "yawnless-create-agent",
          path: "/api/skills/yawnless-create-agent"
        }
      ]
    });
  });
