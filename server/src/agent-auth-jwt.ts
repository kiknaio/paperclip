...
function jwtConfig() {
  const secret = process.env.YAWNLESS_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.YAWNLESS_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.YAWNLESS_AGENT_JWT_ISSUER ?? "yawnless",
    audience: process.env.YAWNLESS_AGENT_JWT_AUDIENCE ?? "yawnless-api",
  };
}
