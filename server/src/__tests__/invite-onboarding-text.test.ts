...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Yawnless.ai OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Yawnless.ai base URLs to try");
    ...
    expect(text).toContain("~/.openclaw/workspace/yawnless-claimed-api-key.json");
    expect(text).toContain("YAWNLESS_API_KEY");
