...
import { execute, testEnvironment } from "@yawnlessai/adapter-openclaw-gateway/server";
import { parseOpenClawGatewayStdoutLine } from "@yawnlessai/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@yawnlessai/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("yawnless:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("YAWNLESS_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("YAWNLESS_TASK_ID=task-123");
