...
describe("agent local JWT", () => {
  const secretEnv = "YAWNLESS_AGENT_JWT_SECRET";
  const ttlEnv = "YAWNLESS_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "YAWNLESS_AGENT_JWT_ISSUER";
  const audienceEnv = "YAWNLESS_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "yawnless";
    process.env[audienceEnv] = "yawnless-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
