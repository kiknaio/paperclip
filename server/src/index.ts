...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.YAWNLESS_SECRETS_PROVIDER === undefined) {
    process.env.YAWNLESS_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.YAWNLESS_SECRETS_STRICT_MODE === undefined) {
    process.env.YAWNLESS_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.YAWNLESS_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.YAWNLESS_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.YAWNLESS_MIGRATION_PROMPT === "never") return false;
    if (process.env.YAWNLESS_MIGRATION_AUTO_APPLY === "true") return true;
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@yawnless.local";
  ...
  const embeddedDbUser = process.env.YAWNLESS_DB_USER?.trim() || process.env.PAPERCLIP_DB_USER?.trim() || "paperclip";
  const embeddedDbPassword =
    process.env.YAWNLESS_DB_PASSWORD?.trim() || process.env.PAPERCLIP_DB_PASSWORD?.trim() || "paperclip";
  const embeddedDbName = process.env.YAWNLESS_DB_NAME?.trim() || process.env.PAPERCLIP_DB_NAME?.trim() || "paperclip";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.YAWNLESS_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    const embeddedAdminConnectionString = `postgres://${embeddedDbUser}:${embeddedDbPassword}@127.0.0.1:${port}/postgres`;
    ...
    const embeddedConnectionString = `postgres://${embeddedDbUser}:${embeddedDbPassword}@127.0.0.1:${port}/${embeddedDbName}`;
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.YAWNLESS_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or YAWNLESS_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.YAWNLESS_LISTEN_HOST = runtimeListenHost;
  process.env.YAWNLESS_LISTEN_PORT = String(listenPort);
  process.env.YAWNLESS_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.YAWNLESS_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.YAWNLESS_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Yawnless.ai server failed to start");
    process.exit(1);
  });
}
